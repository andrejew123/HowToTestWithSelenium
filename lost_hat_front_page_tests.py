...
class LostHatFrontPAgeTests(BaseTestClass):
    ...
    def test_number_of_featured_products(self):
        ...
        self.assertEqual(
            expected_number_of_product_on_page,
            len(product_miniature_xpath),
        ...
        for element in price_element:
            ...
            with self.subTest(price_element_text):
                self.assertIn(
                    "PLN",
                    price_element_text,
                    f'Price of product is not in PLN for {element}'
                )
